...
frame.s_addr = ANOTC_DEVICE_ADDR;\
frame.d_addr = ANOTC_PC_ADDR;\
frame.len = 0;\
}
...
static inline void anotc_add_long(struct anotc_frame *frame, long long data)
{
	*(long long*)(&(frame->data[frame->len])) = data;
	frame->len += sizeof(long long);
}

static inline void anotc_add_ulong(struct anotc_frame *frame, unsigned long long data)
{
	*(unsigned long long*)(&(frame->data[frame->len])) = data;
	frame->len += sizeof(unsigned long long);
}
