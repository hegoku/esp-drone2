...
int icm20948_write_reg(struct bus_dev *dev, unsigned char reg, unsigned char byte)
{
...
{
	return icm20948_write_reg(dev, ICM20948_REG_REG_BANK_SEL, bank);
}
...
int icm20948_prob(struct bus_dev *dev)
{
	...
	icm20948_write_reg(dev, ICM20948_REG_PWR_MGMT_1, 0x80); //reset
	delay_ms(50);
	icm20948_write_reg(dev, ICM20948_REG_PWR_MGMT_1, 0x1); //clock source
	icm20948_write_reg(dev, ICM20948_REG_USER_CTRL, 0x30);	 // I2C master mode and set I2C_IF_DIS
	icm20948_write_reg(dev, ICM20948_REG_INT_PIN_CFG, 0xC0); //INT1 pin is active low
	
	icm20948_set_bank(dev, ICM20948_BANK_2);
	icm20948_write_reg(dev, ICM20948_REG_ODR_ALIGN_EN, 0x01);
	icm20948_write_reg(dev, ICM20948_REG_GYRO_SMPLRT_DIV, 0x00); //sample rate 1125HZ
	icm20948_write_reg(dev, ICM20948_REG_GYRO_CONFIG_1, 0x37); //enable DLPF,  ±2000 dps
	icm20948_write_reg(dev, ICM20948_REG_GYRO_CONFIG_2, 0x1); //8x averaging
	icm20948_write_reg(dev, ICM20948_REG_ACCEL_SMPLRT_DIV_1, 0); //1125HZ
	icm20948_write_reg(dev, ICM20948_REG_ACCEL_SMPLRT_DIV_2, 0); //1125HZ
	icm20948_write_reg(dev, ICM20948_REG_ACCEL_CONFIG, 0x31); //enable DLPF,  ±2g
	// icm20948_write_reg(ICM20948_REG_ACCEL_CONFIG_2, 0x1); // 8x averaging
	icm20948_write_reg(dev, ICM20948_REG_TEMP_CONFIG, 0x3); //1125HZ

	icm20948_set_bank(dev, ICM20948_BANK_0);
	icm20948_write_reg(dev, ICM20948_REG_INT_ENABLE_1, 1);
