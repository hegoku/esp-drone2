...
void angle_q_pid(struct angle_q_pid_param *p)
{
	...
	if (s < 1.0f/1e-6f) {
		att_error_x = 2.0f * p->q_error_b.q1;
		att_error_y = 2.0f * p->q_error_b.q2;
		att_error_z = 2.0f * p->q_error_b.q3;
	} else {
		float theta = 2.0f * atan2f(s, p->q_error_b.q0);
		att_error_x = theta * p->q_error_b.q1 * s;
		att_error_y = theta * p->q_error_b.q2 * s;
		att_error_z = theta * p->q_error_b.q3 * s;
	}
