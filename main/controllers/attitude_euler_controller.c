...
#include "attitude_euler_controller.h"

struct pid_data angle_pid[3];

void init_attitude_euler_controller()
{
	...
	for (int i=0;i<PID_YAW+1;i++) {
		...
		angle_pid[i].err_limit = 0.3;
	}
}

void attitude_euler_controller_update()
{
	...
	if (flight.throttle>0.2f) {
		...
		angle_pid[PID_ROLL].prev_value = flight.attitude.roll;
		angle_pid[PID_PITCH].prev_value = flight.attitude.pitch;
